...
RPC_Response processSetElement(const RPC_Data &data)
{
  ...
  {
    tempOff = 25;
    elementPeriod = 1.5 * 60 * 1000UL; //0.5hr
    elementMillis = millis();
    Serial.print("elementMillis=");
    Serial.println(elementMillis);
  }
  digitalWrite(ELEMENT, enabled);
...
void ElementOn50120Sun()
{
  tempOff = 50;
  elementPeriod = 2 * 60 * 60 * 1000UL; // 2hrs
  if (tempAvg < tempOff & holiday == LOW)
  {
    elementMillis = millis();
...
{
  tempOff = 50;
  elementPeriod = 60 * 60 * 1000UL; // 1hr
  if (tempAvg < tempOff & holiday == LOW)
  {
    elementMillis = millis();
    digitalWrite(ELEMENT, HIGH);
...
{
  tempOff = 50;
  elementPeriod = 90 * 60 * 1000UL; // 1hr
  if (tempAvg < tempOff & holiday == LOW)
  {
    elementMillis = millis();
    digitalWrite(ELEMENT, HIGH);
...
{
  tempOff = 40;
  elementPeriod = 30 * 60 * 1000UL; //0.5hr
  if (tempAvg < tempOff & holiday == LOW)
  {
    elementMillis = millis();
    digitalWrite(ELEMENT, HIGH);
...
void ElementTest2504()
{
  tempOff = 25;
  elementPeriod = 4 * 60 * 1000UL; //0.5hr
  if (tempAvg < tempOff & holiday == LOW)
  {
    elementMillis = millis();
    Serial.print("elementMillis=");
...
//  Serial.print("MAC ID: "); Serial.print(stringMAC); Serial.print(" \t\n");
  ...
  if (millis() > send_millis + loopDelay & m == 0)
  {
    m = 1;
    adcA0 = 0;
    adcA1 = 0;
  ...
  if (l == nn & m == 2)
  {
    Serial.println(""); //Serial.print("l="); Serial.println(l);
    ...
    // Control Circulation Pump
    if (tempA6 > tempA0 + 3)
    { // 50 -1.5*C; pump ON when Manifold A6 is hotter than Mid A0 + 1.5*C
      digitalWrite(PUMP, LOW);
      lastpumpState = LOW;
    } // Pump ON, Fail safe
    else if (tempA6<tempA3 | tempA6<tempA4 | tempA3> tempA4 - 1 | tempA4> 80)
    { // pump OFF if Manifold A6 < OUT A3 or < IN A4 or OUT > IN -1*C or IN > 80*C
      digitalWrite(PUMP, HIGH);
      lastpumpState = HIGH;
    } // Pump Off
    else
    {
      digitalWrite(PUMP, lastpumpState);
    } // pump same, maintain last pump state
...
//    Serial.print(" deltaTime = "); Serial.print(deltaT,2); Serial.print(" ms  ");
    ...
    Serial.print(" luxA8=");
    Serial.print(luxA8);
    Serial.print(" luxPrevious=");
    Serial.print(luxPrevious);
    Serial.print(" elState=");
    Serial.print(elementState);
...
//  ThingSpeak.setField(3,adcA6); // Header adc
  ThingSpeak.writeFields(165321, "141K163R0MBQ1GMI"); // test channel */
    ...
    for (const char *c : attributes)
    {
      ...
      for (int8_t i = 1; i < 5; ++i)
      {
        ...
        if (c == "Day")
          slots[i].Day = stream.readString();
        if (c == "Tim")
          slots[i].Time = stream.readString();
        if (c == "Temp")
          slots[i].Temp = stream.parseFloat();
        if (c == "Dur")
          slots[i].Duration = stream.parseFloat();
        // go to the next slot
      };
  ...
  if (hours > 24 & elementState == LOW & holiday == LOW)
  {
    ESP.restart();
  } // reset every 1 days
...
double analogCalibration(double adcRaw)
{
  ...
  if (X6 != 0)
    adcCalibrated += X6 * pow(adcRaw, 6);
  if (X5 != 0)
    adcCalibrated += X5 * pow(adcRaw, 5);
  if (X4 != 0)
    adcCalibrated += X4 * pow(adcRaw, 4);
  if (X3 != 0)
    adcCalibrated += X3 * pow(adcRaw, 3);
  if (X2 != 0)
    adcCalibrated += X2 * pow(adcRaw, 2);
  if (X != 0)
    adcCalibrated += X * adcRaw;
  if (b != 0)
    adcCalibrated += b;
